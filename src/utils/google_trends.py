...
class GoogleTrends:
    ...
    def get_trends_data_with_region(self, keywords, start_date, end_date, category=None, geo='US', resolution='COUNTRY'):
        ...
        try:
            ...
            self.keywords = keywords
            self.trend_request.build_payload(kw_list=keywords, cat=category, timeframe=timeframe, geo=geo)
            self.data = self.trend_request.interest_by_region(resolution=resolution, inc_low_vol=False, inc_geo_code=False)
    ...
    def get_trends_data_with_region_from_multiple_keywords(self, keywords, start_date, end_date, category=None, geo='US', resolution='COUNTRY'):
        ...
        for i, keywords in enumerate(keywords_batch):
            ...
            # get search result volume
            temp_dataframe = self.get_trends_data_with_region(keywords=keywords, start_date=start_date, end_date=end_date, category=category, geo=geo, resolution=resolution).data
